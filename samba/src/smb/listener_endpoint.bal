...
# Represents a service listener that monitors the SMB location.
public type Listener object {
    ...
    *lang:Listener;
    ...
    public function register(service smbService, string? name) returns error? {
        error? response = ();
        handle serviceName = self.EMPTY_JAVA_STRING;
        if(name is string){
            serviceName = java:fromString(name);
        }
        handle|error result = register(self, self.config,  smbService, serviceName);
        if(result is handle){
